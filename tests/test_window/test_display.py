...
from ..mock.pygame.display import PatchedDisplayModule
...
@pytest.mark.unit
@pytest.mark.usefixtures("patch_display_module", "patch_event_module")
class TestWindowUnit:
    ...
    def test__open__pygame_display_init_and_quit(self, patch_display_module: PatchedDisplayModule) -> None:
        # Arrange
        from py_diamond.window.display import Window
        ...
        with window.open():
            ...
            # Assert
            patch_display_module.init.assert_called()
            patch_display_module.quit.assert_not_called()

        patch_display_module.init.assert_called_once()
        patch_display_module.quit.assert_called_once()

    @pytest.mark.parametrize(
        ("size", "resizable", "fullscreen", "vsync", "expected_flags"),
        [
            pytest.param((0, 0), False, False, False, 0, id="(0, 0), False, False, False, NOFLAGS"),
            pytest.param((0, 0), True, False, False, pygame.RESIZABLE, id="(0, 0), True, False, False, pygame.RESIZABLE"),
            pytest.param((0, 0), False, True, False, pygame.FULLSCREEN, id="(0, 0), False, True, False, pygame.FULLSCREEN"),
            pytest.param((0, 0), False, False, True, 0, id="(0, 0), False, False, True, NOFLAGS"),
            pytest.param((0, 0), True, False, True, pygame.RESIZABLE, id="(0, 0), True, False, True, pygame.RESIZABLE"),
            pytest.param((640, 480), False, False, False, 0, id="(640, 480), False, False, False, NOFLAGS"),
            pytest.param((640, 480), True, False, False, pygame.RESIZABLE, id="(640, 480), True, False, False, pygame.RESIZABLE"),
            pytest.param((640, 480), False, False, True, 0, id="(640, 480), False, False, True, NOFLAGS"),
            pytest.param((640, 480), True, False, True, pygame.RESIZABLE, id="(640, 480), True, False, True, pygame.RESIZABLE"),
            pytest.param((1920, 1080), False, False, False, 0, id="(1920, 1080), False, False, False, NOFLAGS"),
            pytest.param(
                (1920, 1080), True, False, False, pygame.RESIZABLE, id="(1920, 1080), True, False, False, pygame.RESIZABLE"
            ),
            pytest.param((1920, 1080), False, False, True, 0, id="(1920, 1080), False, False, True, NOFLAGS"),
            pytest.param(
                (1920, 1080), True, False, True, pygame.RESIZABLE, id="(1920, 1080), True, False, True, pygame.RESIZABLE"
            ),
        ],
    )
    def test__open__specific_mode(
        ...
        vsync: bool,
        expected_flags: int,
        patch_display_module: PatchedDisplayModule,
    ) -> None:
        # Arrange
        from py_diamond.window.display import Window
        ...
        with window.open():
            ...
            # Assert
            patch_display_module.set_mode.assert_called_once_with(size, flags=expected_flags, vsync=int(vsync))
    ...
    def test__open__do_not_call_pygame_display_quit_on_init_error(
        self, error: type[Exception], patch_display_module: PatchedDisplayModule
    ) -> None:
        # Arrange
        from py_diamond.window.display import Window

        patch_display_module.init.side_effect = error
    ...
    def test__open__always_call_pygame_display_quit_on_internal_setup_error(
        self, error: str, mocker: MockerFixture, patch_display_module: PatchedDisplayModule
    ) -> None:
        # Arrange
        from py_diamond.window.display import Window
        ...
        match error:
            case "set_mode failed":
                patch_display_module.set_mode.side_effect = pygame.error(error)
        ...
        # Assert
        patch_display_module.quit.assert_called()
    ...
    def test__open__do_not_call_window_init_if_set_mode_failed(
        ...
        window = Window()
        patch_display_module.set_mode.side_effect = pygame.error("Something happened")
        mock_window_init = mocker.patch.object(window, "__window_init__")
        mock_window_quit = mocker.patch.object(window, "__window_quit__")
